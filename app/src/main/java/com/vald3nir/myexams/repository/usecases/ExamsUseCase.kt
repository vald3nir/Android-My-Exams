...
internal class ExamsUseCase @Inject constructor(
    ...
    suspend fun updateExam(exam: ExamDTO) {
        analytics.onLog("Exams - updateExam")
        examsDataSource.updateExam(exam)
        updateStateFlow()
    }

    fun getExamById(examId: String) = flow {
        analytics.onLog("Exams - getExamById: $examId")
        emit(
            value = examsDataSource.loadExamById(
                email = userEmail(),
                examId = examId
            )
        )
    ...
    suspend fun deleteExam(examId: String) {
        analytics.onLog("Exams - deleteExam: $examId")
        examsDataSource.delete(examId)
        updateStateFlow()
    }

    suspend fun parseExamFromPdf(pdfPath: String): ExamDTO = withContext(Dispatchers.IO) {
        analytics.onLog("Exams - parseExamFromPdf")
